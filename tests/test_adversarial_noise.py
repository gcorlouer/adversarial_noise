...
class TestModelPredictions:
    ...
    def test_image_to_tensor(self):
        ...
        assert image.shape == (
            ...
            224,
            224,
        ), f"Image shape is {image.shape}, should be {(3, 224, 224)}"
    ...
    def test_model_prediction(self):
        ...
        ), f"Predicted class is {label}, should be chimpanzee, chimp, Pan troglodytes"
        assert proba < 1.0, f"Predicted probability is {proba}, should be less than 1.0"
        assert (
            proba >= 0.9
        ), f"Predicted probability is {proba}, should be greater than 0.9"
...
class TestAdversarialAttack:
    ...
    def test_fgsm_attack(self):
        ...
        assert adversarial_image.shape == (
            1,
            3,
            224,
            224,
        ), f"Adversarial image shape is {adversarial_image.shape}, should be {(3, 224, 224)}"
        adversarial_predictions = model_loader.predict(adversarial_image)
        label = labels.get_label(str(adversarial_predictions[0][0][0]))
        assert (
            label == target_label
        ), f"Adversarial class is {label}, should be {target_label}"
